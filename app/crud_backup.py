...
def create_plan(db: Session, plan: schemas.PlanMasterCreate) -> models.PlanMaster:
    ...
    # Create order links - need to link to order items, not just orders
    for order_id in plan.order_ids:
        ...
        for order_item in order_items:
            order_link = models.PlanOrderLink(
                ...
                quantity_allocated=order_item.quantity_rolls  # Allocate all rolls for now
            )
            db.add(order_link)
    
    # Create inventory links (if any)
